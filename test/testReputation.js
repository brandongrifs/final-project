...
contract('testReputation', function(accounts) {
	...
	beforeEach(async function() {
		...
		//token.mint(100);
		reputation._agent = {from: args._agent};
		reputation.AddToList({from: args._contractor}, "Berkeley, CA");
	});
	...
	describe('TestReputationGeneric', function() {
		it("should be able to deploy token contract", async function() {
			assert.ok(reputation.address);
		});
		it("should initialize owner and agent upon deployment ", async function() {
        	assert.equal(await reputation._owner(), args._owner);
        	assert.equal(await reputation._agent(), args._agent);
        	assert.ok(await reputation.AngelList());
        });
